...
public sealed class ClassController : Controller
{
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        if (User.IsInRole("ADMIN"))
        {
            ...
            // Load dropdowns for Assign Subject modal
            var teachersResult  = await _classService.GetAvailableTeachersAsync(sid);
            var subjectsResult  = await _classService.GetSubjectsAsync(sid);
            var semestersResult = await _classService.GetSemestersAsync(sid);
            ViewBag.TeacherList  = teachersResult.Data?.ToList()  ?? new List<TeacherSummaryDto>();
            ViewBag.SubjectList  = subjectsResult.Data?.ToList()  ?? new List<SubjectSelectDto>();
            ViewBag.SemesterList = semestersResult.Data?.ToList() ?? new List<SemesterSelectDto>();
        }

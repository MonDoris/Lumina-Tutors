...
public sealed class GradingService : IGradingService
{
        ...
        int schoolId, int teacherId, EnterScoreRequest request, CancellationToken ct = default)
    {
        ...
        return Result<ScoreEntryDto>.Success(new ScoreEntryDto(
        ...
        int subjectAssignmentId, CancellationToken ct = default)
    {
        ...
        // Lấy tất cả học sinh đăng ký lớp (không chỉ học sinh đã có điểm)
        var enrollments = await _uow.ClassEnrollments.FindAsync(
            e => e.ClassId == assignment.ClassId,
            ct: ct);
        var studentIds = enrollments.Select(e => e.StudentId).Distinct().ToList();

        // Nếu chưa có enrollment nào, fallback về danh sách từ score entries
        if (!studentIds.Any())
            studentIds = assignment.ScoreEntries.Select(se => se.StudentId).Distinct().ToList();
        ...
        int studentId, int subjectAssignmentId, CancellationToken ct = default)
    {
        ...
        var dtxScores = entries
            .Where(se => se.GradeCategory?.CategoryCode == "DTX")
            .Select(se => se.Score).ToList();
        var dgk = entries.FirstOrDefault(se => se.GradeCategory?.CategoryCode == "DGK")?.Score;
        var dck = entries.FirstOrDefault(se => se.GradeCategory?.CategoryCode == "DCK")?.Score;

        // Tính ĐTBm theo TT22: (Σ DTX×1 + DGK×2 + DCK×3) / (số DTX + 5)
        decimal? average = null;
        if (dtxScores.Any() && dgk.HasValue && dck.HasValue)
        {
            var numerator   = dtxScores.Sum() + dgk.Value * 2 + dck.Value * 3;
            var denominator = dtxScores.Count + 5;
            average = Math.Round(numerator / denominator, 1);
        }

        // Lưu hoặc cập nhật GradeBook
        var assignment = await _uow.SubjectAssignments.GetByIdAsync(subjectAssignmentId, ct);
        var gradeBooks = await _uow.GradeBooks.FindAsync(
        ...
        if (gb is null)
        {
            gb = new Domain.Entities.Grading.GradeBook
            {
                SchoolId            = assignment?.SchoolId ?? 0,
                StudentId           = studentId,
                SubjectAssignmentId = subjectAssignmentId,
                AverageScore        = average,
                Remark              = average.HasValue ? ToRemark(average.Value) : (Domain.Enums.GradeRemark?)null,
                IsCalculated        = average.HasValue,
                IsLocked            = false
            };
            await _uow.GradeBooks.AddAsync(gb, ct);
        }
        else
        {
            gb.AverageScore = average;
            gb.Remark       = average.HasValue ? ToRemark(average.Value) : (Domain.Enums.GradeRemark?)null;
            gb.IsCalculated = average.HasValue;
        }
        await _uow.SaveChangesAsync(ct);
        ...
        var regularScores = dtxScores.Select(s => (decimal?)s).ToList();
    ...
    private static Domain.Enums.GradeRemark ToRemark(decimal avg) => avg switch
    {
        >= 9.0m => Domain.Enums.GradeRemark.Excellent,
        >= 7.0m => Domain.Enums.GradeRemark.Good,
        >= 5.0m => Domain.Enums.GradeRemark.Average,
        >= 3.5m => Domain.Enums.GradeRemark.BelowAvg,
        _       => Domain.Enums.GradeRemark.Fail
    };
}

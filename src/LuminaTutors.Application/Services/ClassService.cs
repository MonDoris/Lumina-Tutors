...
public sealed class ClassService : IClassService
{
    ...
    public async Task<Result<IReadOnlyList<SemesterSelectDto>>> GetSemestersAsync(
        int schoolId, CancellationToken ct = default)
    {
        var semesters = await _uow.Semesters.FindAsync(
            s => s.SchoolId == schoolId, ct: ct);

        var dtos = semesters
            .OrderByDescending(s => s.IsActive)
            .ThenByDescending(s => s.StartDate)
            .Select(s => new SemesterSelectDto(s.Id, s.SemesterName, s.IsActive))
            .ToList() as IReadOnlyList<SemesterSelectDto>;

        return Result<IReadOnlyList<SemesterSelectDto>>.Success(dtos);
    }
